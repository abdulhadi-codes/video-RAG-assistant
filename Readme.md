...
Flask API (app.py)
        ↓
RAG Pipeline (src/)
   ├── transcription.py   — audio extraction + Whisper transcription
   ├── chunking.py        — timestamped transcript chunking
   ├── embeddings.py      — embedding generation (indexing + query time)
   ├── vector_store.py    — Joblib-backed vector storage
   ├── retrieval.py       — cosine similarity top-K retrieval
   └── generation.py      — prompt construction + LLM answer generation
```
...
> **Note:** the embedding step and the LLM generation step each support more than one backend in this repository — a locally-running option (Ollama) alongside a hosted-API option (fastembed for embeddings, Groq for generation). Confirm in `src/embeddings.py` and `src/generation.py` which is currently wired into `app.py` in your deployed version.
...
├── config.py                  # Local API key config (not committed to git)
│
├── src/
│   ├── __init__.py
│   ├── transcription.py       # FFmpeg audio extraction + Whisper transcription
│   ├── chunking.py            # Builds timestamped transcript chunks
│   ├── embeddings.py          # Embedding generation (indexing + query time)
│   ├── vector_store.py        # Loads/saves the embedded dataset via Joblib
│   ├── retrieval.py           # Cosine similarity top-K chunk retrieval
│   └── generation.py          # Prompt construction + LLM answer generation
│
├── templates/
